"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.GroupSchema = exports.Group = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose_2 = require("mongoose");
let GroupMember = class GroupMember {
};
__decorate([
    (0, mongoose_1.Prop)({ type: mongoose_2.Schema.Types.ObjectId, ref: 'User', required: true }),
    __metadata("design:type", mongoose_2.Schema.Types.ObjectId)
], GroupMember.prototype, "userId", void 0);
__decorate([
    (0, mongoose_1.Prop)({
        type: String,
        enum: ['admin', 'member'],
        default: 'member',
    }),
    __metadata("design:type", String)
], GroupMember.prototype, "role", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: Date, default: Date.now }),
    __metadata("design:type", Date)
], GroupMember.prototype, "joinedAt", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: [String], default: [] }),
    __metadata("design:type", Array)
], GroupMember.prototype, "permissions", void 0);
GroupMember = __decorate([
    (0, mongoose_1.Schema)({ _id: false })
], GroupMember);
let GroupSettings = class GroupSettings {
};
__decorate([
    (0, mongoose_1.Prop)({
        type: String,
        enum: ['public', 'private'],
        default: 'public',
    }),
    __metadata("design:type", String)
], GroupSettings.prototype, "privacy", void 0);
__decorate([
    (0, mongoose_1.Prop)({ default: false }),
    __metadata("design:type", Boolean)
], GroupSettings.prototype, "joinApproval", void 0);
__decorate([
    (0, mongoose_1.Prop)({ default: true }),
    __metadata("design:type", Boolean)
], GroupSettings.prototype, "allowInvites", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: Number, default: 30 }),
    __metadata("design:type", Number)
], GroupSettings.prototype, "messageRetention", void 0);
GroupSettings = __decorate([
    (0, mongoose_1.Schema)({ _id: false })
], GroupSettings);
let GroupMetadata = class GroupMetadata {
};
__decorate([
    (0, mongoose_1.Prop)({ type: Date, default: Date.now }),
    __metadata("design:type", Date)
], GroupMetadata.prototype, "createdAt", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: Date, default: Date.now }),
    __metadata("design:type", Date)
], GroupMetadata.prototype, "updatedAt", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: Date }),
    __metadata("design:type", Date)
], GroupMetadata.prototype, "lastActivityAt", void 0);
GroupMetadata = __decorate([
    (0, mongoose_1.Schema)({ _id: false })
], GroupMetadata);
let Group = class Group extends mongoose_2.Document {
};
exports.Group = Group;
__decorate([
    (0, mongoose_1.Prop)({
        required: true,
        trim: true,
        minlength: 3,
        maxlength: 50,
    }),
    __metadata("design:type", String)
], Group.prototype, "name", void 0);
__decorate([
    (0, mongoose_1.Prop)({
        trim: true,
        maxlength: 500,
    }),
    __metadata("design:type", String)
], Group.prototype, "description", void 0);
__decorate([
    (0, mongoose_1.Prop)({ required: false }),
    __metadata("design:type", String)
], Group.prototype, "avatar", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: mongoose_2.Schema.Types.ObjectId, ref: 'User', required: true }),
    __metadata("design:type", mongoose_2.Schema.Types.ObjectId)
], Group.prototype, "owner", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: [GroupMember], default: [] }),
    __metadata("design:type", Array)
], Group.prototype, "members", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: GroupSettings, default: () => ({}) }),
    __metadata("design:type", GroupSettings)
], Group.prototype, "settings", void 0);
__decorate([
    (0, mongoose_1.Prop)({ type: GroupMetadata, default: () => ({}) }),
    __metadata("design:type", GroupMetadata)
], Group.prototype, "metadata", void 0);
exports.Group = Group = __decorate([
    (0, mongoose_1.Schema)({
        collection: 'groups',
        timestamps: true,
    })
], Group);
exports.GroupSchema = mongoose_1.SchemaFactory.createForClass(Group);
//# sourceMappingURL=group.schema.js.map