...
describe('UploadController (e2e)', () => {
  ...
  it('/POST upload file (successful)', async () => {
    ...
    const response = await request(app.getHttpServer()).post('/upload').attach('file', mockFile, 'test-file.jpg').expect(201);
  ...
  it('/POST upload file (error handling)', async () => {
    ...
    mockUploadService.uploadFile.mockRejectedValue(new Error('Internal server error'));

    // Create a mock file buffer
    const mockFile = Buffer.from('test file content');

    const response = await request(app.getHttpServer()).post('/upload').attach('file', mockFile, 'test-file.jpg').expect(500);
