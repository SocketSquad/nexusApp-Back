...
export class GroupMessagesController {
  ...
  async findByConversation(@Param('conversationId') conversationId: string, @Query('page') page: number = 1, @Query('limit') limit: number = 50) {
  ...
  async update(@Param('id') id: string, @Body() updateMessageDto: UpdateGroupMessageDto, @Body('userId') userId: string) {
