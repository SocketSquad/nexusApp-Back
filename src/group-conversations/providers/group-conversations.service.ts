import { 
  ...
  export class GroupConversationsService implements IGroupConversationsService {
    constructor(
      private readonly groupConversationsRepository: GroupConversationsRepository,
    ) {}
    ...
    async create(
      ...
      const creatorParticipant = createConversationDto.participants.find(
      ...
      const uniqueParticipants = new Set(
        createConversationDto.participants.map(p => p.userId)
      );
    ...
    async addParticipant(
      ...
      const isExistingParticipant = conversation.participants.some(
      ...
      return this.groupConversationsRepository.addParticipant(
        conversationId, 
        participantId
      );
    }
    ...
    async removeParticipant(
      ...
      // Check if user has admin permissions
      const userRole = conversation.participants.find(
        p => p.userId.toString() === removedById
      )?.role;
      ...
      return this.groupConversationsRepository.removeParticipant(
        conversationId, 
        participantId
      );
    }
