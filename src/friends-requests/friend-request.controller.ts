...
export class FriendRequestController {
  ...
  @Get(':userId')
  async findAllForUser(@Param('userId') userId: string): Promise<FriendRequest[]> {
    return this.friendRequestService.findAllByUser(userId);
  }

  @Patch(':id/accept')
  async acceptRequest(@Param('id') id: string): Promise<FriendRequest> {
    return this.friendRequestService.acceptRequest(id);
  }

  @Patch(':id/reject')
  async rejectRequest(@Param('id') id: string): Promise<FriendRequest> {
    return this.friendRequestService.rejectRequest(id);
  }

  @Delete(':id')
  async cancelRequest(@Param('id') id: string): Promise<FriendRequest> {
