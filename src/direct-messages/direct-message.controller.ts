import {
  ...
  export class DirectMessageController {
    ...
    async createMessage(
      @Body(ValidationPipe) createMessageDto: CreateDirectMessageDto,
    ...
    async getConversationMessages(
      ...
      @Query(ValidationPipe) query: MessageQueryDto,
    ): Promise<DirectMessage[]> {
      const { limit = 50, before } = query;
      return this.directMessageService.findByConversation(
    ...
    async updateMessage(
      @Param('id') id: string,
      @Body(ValidationPipe) updateMessageDto: UpdateDirectMessageDto,
    ): Promise<DirectMessage> {
      const updateData = {
        ...updateMessageDto,
    ...
    async getMessageCount(
      ...
      return this.directMessageService.getMessageCount(
        new Types.ObjectId(conversationId),
      );
    }
    ...
    async getLatestMessage(
      ...
      return this.directMessageService.getLatestMessage(
        new Types.ObjectId(conversationId),
      );
    }
  }